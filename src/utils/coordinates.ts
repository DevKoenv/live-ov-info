/**
 * Converts Dutch RD coordinates to WGS84 (GPS) coordinates
 * 
 * @param rdX - X-coordinate in the RD coordinate system (in meters)
 * @param rdY - Y-coordinate in the RD coordinate system (in meters)
 * @returns An object containing the latitude and longitude in WGS84
 */
export function convertRdToWgs84(rdX: number | string, rdY: number | string): { latitude: number; longitude: number } | { error: Error } {
  let x = parseFloat(rdX as string);
  let y = parseFloat(rdY as string);
...
/**
 * Internal conversion function for RD to WGS84
 */
const convert = (x: number, y: number): { latitude: number; longitude: number } => {
  const x0 = 155000.000;
  const y0 = 463000.000;
  ...
  const latitude = f + (-96.862 - 11.714 * (f - 52) - 0.125 * (l - 5)) / 100000;
  const longitude = l + (-37.902 + 0.329 * (f - 52) - 14.667 * (l - 5)) / 100000;
